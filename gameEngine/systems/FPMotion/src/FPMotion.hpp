...
namespace GameEngine {
     ...
     * 1. **Deceleration Phase**: Reduces velocity by 75% when no external force is applied
     ...
     * @see Renderable
     */
    class FPMotion : public System<FPMotion> {
    public:
        /**
         ...
         */
        FPMotion() {
            requireComponents<GameEngine::Position, GameEngine::Velocity, GameEngine::Acceleration, GameEngine::Renderable, GameEngine::Gravity>();
        }
        
        /**
         * @brief Updates all entity positions based on physics simulation.
         *
         * Processes each entity with motion components through a three-phase physics pipeline:
         *
         ...
         * - Preserves sign: positive velocities stay positive, negative stay negative
         ...
         *
         * @param registry Reference to the ECS Registry (unused but required by interface).
         * @param dt Delta time since last update in seconds (unused in current implementation).
         *
         * @details
        ...
        void onUpdate(Registry& registry, float dt) {
            ...
            registry.each<Position, Velocity, Acceleration, Renderable, Gravity>([dt](auto e, Position& pos, Velocity& vel, Acceleration& acc, Renderable& render, Gravity& grav) {
                ...
                // Phase 3: Position update (translate and constrain to screen bounds)
                ...
                std::cout << "Acc: " << acc.x << ", " << acc.y << "\n" << "Vel: " << vel.x << ", " << vel.y << "\n" << "Pos: " << pos.pos.x << ", " << pos.pos.y << std::endl;
            });
        }
         ...
         *
         * @remarks Value persists throughout system lifetime; reset manually if needed for testing.
         */
        int updateCount = 0;
    };
}
