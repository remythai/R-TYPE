...
#include "../../../components/gravity/src/Gravity.hpp"
...
namespace GameEngine {
     ...
     *
     * The Death system iterates through all entities that possess a Health component
     * and automatically destroys any entity whose `currentHp` reaches zero. This ensures
     * that dead entities are cleaned up from the ECS registry automatically each frame.
     *
     * @details
     ...
     *
     * This approach decouples the death logic from damage dealing, allowing flexible
     * death handling without tight coupling.
     *
     * @note An entity is only destroyed if `currentHp == 0` (exact equality).
     *       This allows for potential "undead" mechanics where health might go negative.
     *
     * @performance
     * - Time Complexity: O(n) where n is the number of entities with Health components
     ...
     */
    class FPApplyGravity : public System<FPApplyGravity> {
    public:
        /**
         * @brief Constructs the Death system and registers its component requirements.
         *
         * During construction, the system declares that it requires the Health component.
         * The ECS registry will only activate this system when at least one entity with
         * a Health component exists.
         *
         ...
         */
        FPApplyGravity() {
            requireComponents<GameEngine::Acceleration, GameEngine::Gravity, GameEngine::InputControlled>();
        }
        
        /**
         * @brief Processes all entities with Health components and destroys those with zero HP.
         *
         ...
         *
         * @attention Destroying entities during iteration is safe because the ECS system
         *            uses optimized pool iteration that doesn't invalidate cursors.
         *
         * @warning If an entity's health goes below zero (negative HP), it will NOT be
         *          destroyed by this system. Ensure damage systems clamp health to 0.
         *
         * @example
         * ```cpp
         * Registry registry;
        ...
        void onUpdate(Registry& registry, float dt) {
            ...
            registry.each<Acceleration, Gravity, InputControlled>(
                [dt, &registry](auto e, Acceleration& acc, Gravity& grav, InputControlled& input) {
                    if (input.firstInput) {
                        acc.y = grav.force;
                    }
                }
