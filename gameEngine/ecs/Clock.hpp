...
namespace GameEngine {
    ...
    * while tracking total elapsed time, frame count, and interpolation alpha for rendering.
    *
    ...
    */
    struct GameClock {
        ...
        /// @brief Time scaling factor (1.0 = normal speed, 0.5 = half speed, 2.0 = double speed).
        ...
        *
        * The function increments the accumulator using real time (`realDt * timeScale`)
        * and determines how many fixed-timestep updates should occur during this frame.
        * 
        * It caps the number of fixed updates per frame to prevent a "spiral of death"
        * (where the simulation tries to catch up indefinitely after a large frame hitch).
        *
        ...
        * `interpolated_position = lerp(previous, current, getInterpolationAlpha())`
        *
