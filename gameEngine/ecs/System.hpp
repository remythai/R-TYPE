 ...
 *
 * Systems define logic that operates on entities with specific component
 * combinations. Each system updates once per frame (or tick) and processes
 * entities that match its required component signature.
 *
 * This abstract base defines the common interface that all derived systems must
 * implement.
 */
class ISystem
{
   public:
     ...
     *
     * Implementations should define how entities are processed using the
     * `Registry`.
     *
    ...
    /// @brief Indicates whether this system’s required components are
    /// registered.
...
/**
 * @class System
 * @brief CRTP (Curiously Recurring Template Pattern) base class for ECS
 * systems.
 *
...
class System : public ISystem
{
   ...
   protected:
     ...
     *
     * Useful for runtime configuration (e.g., editor tools or scripting
     * systems).
     *
   ...
   private:
    /// @brief Bitset defining which components an entity must have to match
    /// this system.
