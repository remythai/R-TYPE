...
void CLIENT::Core::setupNetworkCallbacks()
{
    _networkClient->setOnPlayerIdReceived([this](uint8_t playerId) {
        handlePlayerIdReceived(playerId);
    });
...
float CLIENT::Core::readFloat(const std::vector<uint8_t>& payload, size_t& offset)
{
...
bool CLIENT::Core::parseSnapshotEntity(const std::vector<uint8_t>& payload, 
                                       size_t& offset,
                                       std::set<uint8_t>& activeEntities)
{
    ...
    if (offset + pathLen > payload.size()) return false;
    ...
    updateOrCreateEntity(entityId, x, y, spritePath, rectPosX, rectPosY, rectSizeX, rectSizeY);
    
    return true;
}

void CLIENT::Core::updateOrCreateEntity(uint8_t entityId, float x, float y,
                                        ...
                                        float rectSizeX, float rectSizeY)
{
    ...
    if (entity) {
        ...
        updateEntitySprite(entity, entityId, spritePath, needsNewSprite,
                          rectPosX, rectPosY, rectSizeX, rectSizeY);
    }
}
...
void CLIENT::Core::updateEntitySprite(GameEntity* entity, uint8_t entityId,
                                      const std::string& spritePath,
                                      bool needsNewSprite,
                                      float rectPosX, float rectPosY,
                                      float rectSizeX, float rectSizeY)
{
    ...
    if (entity->sprite.has_value()) {
        applySpriteTransform(entity->sprite.value(), rectPosX, rectPosY, 
                           rectSizeX, rectSizeY, entity->position);
    }
}

sf::Texture* CLIENT::Core::findTexture(const std::string& spritePath, uint8_t entityId)
{
    auto& rm = ResourceManager::getInstance();
...
void CLIENT::Core::applySpriteTransform(sf::Sprite& sprite,
                                        float rectPosX, float rectPosY,
                                        float rectSizeX, float rectSizeY,
...
bool CLIENT::Core::shouldCleanupEntities(const std::chrono::steady_clock::time_point& lastCleanup,
                                         float interval)
{
