...
void CLIENT::MapEditor::renderEntities(sf::RenderWindow& window)
{
    ...
    for (size_t i = 0; i < _entities.size(); ++i) {
        ...
        if (texture) {
            ...
            sprite.setPosition(sf::Vector2f(entity.x - entity.textureRect.size.x / 2.0f, 
            ...
            if (static_cast<int>(i) == _selectedEntityIndex) {
                sf::RectangleShape selectionBox(
                    sf::Vector2f(entity.textureRect.size.x * 2.0f, 
                                entity.textureRect.size.y * 2.0f)
                );
                selectionBox.setPosition(sf::Vector2f(entity.x - entity.textureRect.size.x, 
...
void CLIENT::MapEditor::renderPreview(sf::RenderWindow& window)
{
    ...
    if (texture) {
        ...
        preview.setPosition(sf::Vector2f(_mouseWorldPos.x - textureRect.size.x, 
        ...
        previewShape.setFillColor(sf::Color(getEntityColor(_selectedEntity).r,
                                        getEntityColor(_selectedEntity).g,
                                        getEntityColor(_selectedEntity).b,
                                        128));
    ...
    if (ImGui::BeginListBox("##entities", ImVec2(-1, 150))) {
        for (size_t i = 0; i < _entities.size(); ++i) {
            ...
            snprintf(label, sizeof(label), "%s @ (%.0f, %.0f) t=%.2fs",
                    getEntityName(entity.type).c_str(),
                    entity.x, entity.y, entity.spawnTime);
    ...
    if (leftMouseIsPressed && !leftMouseWasPressed) {
        ...
        switch (_selectedEntity) {
            case EntityTemplate::ENEMY_TYPE1:
                ...
                entity.textureRect = sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(33.31, 36));
                break;
            case EntityTemplate::ENEMY_TYPE2:
                entity.spritePath = "assets/sprites/r-typesheet9.png";
                entity.textureRect = sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(55.33, 54));
                break;
            case EntityTemplate::ENEMY_TYPE3:
                entity.spritePath = "assets/sprites/r-typesheet10.png";
                entity.textureRect = sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(33.17, 30));
                break;
            case EntityTemplate::ENEMY_TYPE4:
                entity.spritePath = "assets/sprites/r-typesheet11.png";
                entity.textureRect = sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(33.33, 34));
                break;
            default:
                entity.spritePath = "assets/sprites/r-typesheet42.png";
                entity.textureRect = sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(33.33, 36));
                break;
        }
...
void CLIENT::MapEditor::saveMap(const std::string& filename)
{
    ...
    for (size_t i = 0; i < _entities.size(); ++i) {
        ...
        file << "      \"textureRect\": [" 
             << entity.textureRect.position.x << ", "
             << entity.textureRect.position.y << ", "
             << entity.textureRect.size.x << ", "
             << entity.textureRect.size.y << "]\n";
        file << "    }";
        if (i < _entities.size() - 1) file << ",";
        file << "\n";
    }
...
void CLIENT::MapEditor::loadMap(const std::string& filename)
{
    ...
    _entities.clear();
    std::cout << "Map loading from " << filename << " (implement proper JSON parsing)\n";
